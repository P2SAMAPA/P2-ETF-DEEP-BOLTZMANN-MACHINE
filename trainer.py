...
import data_manager
import push_results
from dbm_engine import compute_dbm_scores
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (DBM Engine) ===")
        ...
        for win in config.WINDOWS:
            min_bars = win + config.N_LAGS + 21 + 10
        ...
        top_etfs    = [
            {"ticker": t, "dbm_score": float(s), "best_window": int(w)}
        ...
        for win, score_dict in window_results.items():
            ...
            windows_tab2[str(win)] = {
                "top_etfs":    [{"ticker": t, "dbm_score": float(s)} for t, s in sw[:config.TOP_N]],
    ...
    print(f"\n=== DBM Engine complete: {tab1_path.name}, {tab2_path.name} ===")
